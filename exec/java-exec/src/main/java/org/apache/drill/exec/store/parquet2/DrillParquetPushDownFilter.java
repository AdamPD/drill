...
public class DrillParquetPushDownFilter extends StoragePluginOptimizerRule {
    private static final Logger logger = LoggerFactory
            .getLogger(DrillParquetPushDownFilter.class);
    public static final StoragePluginOptimizerRule INSTANCE = new DrillParquetPushDownFilter();

    private DrillParquetPushDownFilter() {
        super(
    ...
    public void onMatch(RelOptRuleCall call) {
        ...
        DrillParquetFilterBuilder parquetFilterBuilder = new DrillParquetFilterBuilder(groupScan,
                conditionExp);
        ...
        if (parquetFilterBuilder.areAllExpressionsConverted()) {
      /*
       * Since we could convert the entire filter condition expression into a
       * Parquet filter predicate, we can eliminate the filter operator altogether.
       */
