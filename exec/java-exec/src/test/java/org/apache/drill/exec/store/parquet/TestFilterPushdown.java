...
public class TestFilterPushdown extends PlanTestBase {
    ...
    public void testSimpleQuery() throws Exception {
        ...
        String query = String.format("select count(*) `count` from dfs_test.`%s/parquet/pushdown` where price > 10.0", TEST_RES_PATH);
        testBuilder()
            .ordered()
            .sqlQuery(query)
            .baselineColumns("`count`")
    ...
    public void testEmptyQuery() throws Exception {
        ...
        String query = String.format("select count(*) `count` from dfs_test.`%s/parquet/pushdown` where price > 100.0", TEST_RES_PATH);
        testBuilder()
            .ordered()
            .sqlQuery(query)
            .baselineColumns("`count`")
